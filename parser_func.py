...
PARAM_PATTERN = {'Количество комнат':'rooms',
                 'Площадь обща':'square',
                 'Год постройки':'year'
...
def get_last_page() -> int:
    ...
    last_page = soup.find_all('a', class_='focus:outline-none sm:focus:shadow-10bottom cursor-pointer select-none inline-flex font-normal text-body min-h-[2.5rem] min-w-[2.5rem] py-0 items-center !px-1.25 justify-center mx-1 hidden md:inline-flex hover:bg-basic-200 rounded-md disabled:text-basic-500')
...
def get_all_links(last_page: int) -> list:
    ...
    for page in tqdm(range(1, last_page + 1), desk='PARSER URLS: '):
...
def get_flats_data(links: list) -> dict:
    ...
    for link in tqdm( links, desc='parsing data: '):
        flat = {'rooms':'', 'square':'', 'year':'', 'floor':'', 'type_house':'','region':'', 'cyti':'', 'street':'', 'district':'', 'coordinates':''}
        ...
        title = s.find('h1', {"class": "order-1 mb-0.5 md:-order-2 md:mb-4 block w-full !inline-block lg:text-h1Lg text-h1 font-raleway font-bold flex items-center"}).text
        try:
            price = s.find('h2',
...
def run_parser():
    ...
    data = get_flats_dat(links)
    for iten in data.values():
        insert_flat(iten)
